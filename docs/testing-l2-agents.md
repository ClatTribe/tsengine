...
| **cloud_account** | `internal/cloudagent` | VulnAgent-style investigate loop (§10): the model drives, calling deterministic cloud tools (`cloudgraph` reachability, `cloudiam` effective-perms, the attack-path enumerator) | `cloudengine.LLM` | enumerates real attack paths, every claim backed by a tool result; remediations re-verified through `cloudiam.Authorize` before a PR/ticket. **UX: `/cloud-engineer`** + `POST /v1/cloud/investigate` (LLM-gated; results stored as findings) + CLI `cloud-investigate`. |
