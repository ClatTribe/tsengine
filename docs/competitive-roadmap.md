...
| **F3** ◑ | **Public self-serve signup + billing** (vs operator-token provisioning) — needs an identity + billing model decision, **flagged for the user**. Verifiable increment shipped instead: a **ServiceNow Filer** (`connector.ServiceNow`, Table API `/api/now/table/incident`, Basic auth) implementing `remediate.Filer`, wired behind `SERVICENOW_INSTANCE_URL` (else-if after Jira) — ticketing parity for ServiceNow shops. | ✅ ServiceNow filer unit-verified (incident POST, auth, error); self-serve signup + billing remains a product decision |
