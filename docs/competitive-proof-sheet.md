...
|---|---|---|---|
| **AppSec** — Snyk, Semgrep, GitHub AS | `semgrep` (SAST), `codeql` (taint depth), `gitleaks`+`trufflehog` (secrets), `osv-scanner`+`trivy`+`grype` (SCA — same OSV/CVE DBs Snyk queries), `mobsfscan` | Same engines → recall parity. **SAST 46.54% Youden** on OWASP Benchmark v1.2, all 2,740 cases — third on the published cohort, just BELOW Checkmarx (47) and above Fortify (35). (SCOREBOARD.md is authoritative; an earlier 47.86% predated the neutral measurement and inverted this comparison.) | L1.5 corroboration (two secret scanners agree → confidence), FP-filter, and `remediate` opens a **fix-PR** (tier-1 auto). They leave triage + fix to the engineer. |
...
| **Compliance automation** — Vanta, Drata, Sprinto, Scrut | `prowler`+`scout-suite` (CIS cloud), `checkov` (IaC), `trivy`/`grype` (vuln evidence) feeding control state | Same CSPM/IaC OSS → evidence parity. The *mapping* is in-house `grc`, spanning **25 frameworks** (SOC2, CIS v8, NIST CSF, ISO 27001, PCI-DSS, HIPAA, SOX, GLBA, GDPR, ISO 27701, CCPA, DPDP, PIPEDA, ISO 27018, 800-53, 800-171, FedRAMP, CMMC, CERT-In, RBI, SEBI, ISO 22301, ISO 42001, NIST AI RMF, EU AI Act) across **three emission paths** (appsec CWE crosswalk, identity findings, cloud attack-paths) — the breadth Vanta/Drata lead with. CIS-baseline scorer ready vs Prowler/Scout. | A control is a "gap" **only because a real finding cites it** (grounded). Then propose→gate→apply→**signed, replayable evidence pack** — auditor/insurer-grade, which Vanta/Drata can't produce. They flag a gap and hand a human a ticket; we also **respond**: a critical incident yields gated **containment** (suspend/restrict/block) + a breach-disclosure draft. |
...
|---|---|---|---|
| Repository · SAST (OWASP Benchmark v1.2) | **46.54% Youden** | Veracode 51 · **Checkmarx 47** · Fortify 35 · SonarQube 6 | ✅ third on the cohort — just below Checkmarx, above Fortify (live, host semgrep) |
...
- **Remediation breadth is partial.** Live gated writes exist for **repo PRs** and **Okta suspend**; cloud / GWorkspace / M365 writes are honest stubs pending admin-write credentials (they surface a clear error, never a false "done"). **Incident containment** is a gated runbook today (suspend/restrict/block, human-approved) carrying a machine-readable target, promotable to a live apply as each connector's write path lands.
- **SOC depth is the deterministic slice.** The detect/incident backbone + gated containment + breach-draft are built and tested; open-ended LLM-driven triage (forensics, multi-step playbooks) is the documented next tranche, not a current claim.
