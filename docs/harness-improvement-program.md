...
| 3a | expanded dev-set baseline (12ch, iter-1 config) | hy3-free(proxy) | **INVALID** | — | — | **BRAIN-DEGRADED WINDOW**: after ~2h of sustained load, every attempt became a 0-turn kill (last-line = OSS-sandbox spawn print; agent never reached turn 1). Tiny health probes stayed green throughout — probe-before/after is NECESSARY BUT NOT SUFFICIENT; sustained-load windows need in-batch detection (sentinel aborts on 3× 0-turn streak — shipped in campaign driver). Free-tier channel verdict: usable for short batches with cool-down gaps, NOT for multi-hour campaigns |
| 3b | reproduction of iter-1 config (6ch) | hy3-free(proxy) | aborted | aborted | — | **ABORTED — infra**: (a) timeout-killed runs LEAKED both sandboxes (SIGKILL skips defer Destroy); leaks cascaded into spawn failures across later challenges — fixed with per-challenge invocation + boundary reap (`run-xbow-v4.sh` pattern); (b) post-fix, agent loop stalled on model calls with 0 turns at repeated ~300s marks while tiny-probe health checks passed before/after ⇒ sustained-load throttle windows on the free tier make tonight's channel unusable for attribution-grade iteration |
