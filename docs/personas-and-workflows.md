...
| **T3 — irreversible-action invariant** (agent prepares, human signs) | ✅ enforcement | `platform.TierIrreversible`=3 + `NeedsHumanSignature()`; `hitl.Desk` refuses to apply a T3 action without a named human approver (`ErrNeedsHumanSignature`) — never on `auto`. *No T3-emitting flow exists yet* (breach-notification / customer-comms ride the future **A-RSP** capability), so the invariant is forward-compatible: the moment a T3 action is produced, it's safe by construction |
