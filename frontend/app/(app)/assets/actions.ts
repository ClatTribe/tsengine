...
export async function rescanAll(): Promise<{ scanned?: number; queued?: boolean; error?: string }> {
  try {
    ...
    // The platform refuses a scan while the kill-switch is engaged (409) and says why. Without
    // catching it the button would spin and settle back to "Scan now" with no explanation, which
    // is the same silence the server fix exists to remove — one layer up.
    return { error: e instanceof ApiError ? e.message : "Could not start the scan." };
  }
}
