...
// POST { workspace, email, password, name?, source? } → create a workspace + its owner account on
// the platform, then sign in (store the returned session token + tenant in httpOnly cookies).
export async function POST(req: Request) {
  const { workspace, email, password, name, source } = await req.json().catch(() => ({}));
  ...
  const res = await fetch(`${apiBase()}/v1/auth/signup`, {
    ...
    body: JSON.stringify({ workspace, email, password, name, source }),
    cache: "no-store",
  }).catch(() => null);
