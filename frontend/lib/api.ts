...
export const api = {
  ...
  // The tenant's own eval suite. Note the fallback has NO agreement key — an empty suite must not
  // render as a perfect score.
  tenantEval: () =>
    safe<TenantEval>("/v1/eval", { cases: 0, passed: 0, failures: [], by_source: {} }),
  ...
  scanDatabase: (dsn: string, sampleValues: boolean) =>
    call<DatabaseScanResult>("/v1/database/scan", {
      method: "POST",
      body: JSON.stringify({ dsn, sample_values: sampleValues }),
    }),
  ...
  // Severity counts for the risk badge. Deliberately NOT api.findings(): the shell renders on every
  // navigation, and a workspace that imported its scanner backlog would pay 27MB each time.
  findingsSummary: () =>
    safe<FindingsSummary>("/v1/findings/summary", { total: 0, severity: {}, truncated: false }),
