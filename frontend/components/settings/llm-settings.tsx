...
const PROVIDERS = [
  { id: "anthropic", label: "Anthropic (Claude)", placeholder: "claude-opus-4-8" },
  { id: "openai", label: "OpenAI", placeholder: "gpt-4o" },
  { id: "gemini", label: "Google (Gemini)", placeholder: "gemini-2.0-flash" },
];

export function LLMSettings({ initial }: { initial: { provider: string; model: string; has_key: boolean } }) {
  ...
  function save() {
    ...
    start(async () => {
      try {
        const r = await setLLMConfig(provider, model.trim(), apiKey.trim());
        setHasKey(r.has_key);
        setApiKey("");
        setSaved(true);
      } catch (e) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted">
        Bring your own LLM for the AI agent and autonomous pentest. Your key is encrypted at rest and never shown again.
      </p>
      <div className="flex flex-wrap items-end gap-3">
        ...
        <label className="text-xs text-muted">
          API key {hasKey && <span className="text-pulse">· set</span>}
          <input
            type="password"
            value={apiKey}
