...
| **repository** | SAST | **OWASP Benchmark v1.2** (BenchmarkJava, 2,740 cases) | Veracode 51 / Checkmarx 47 / Fortify 35 / SonarQube 6 | ✅ `strix-bench/owasp-benchmark:v1.2` | **✅ live — overall Youden 46.54% reproduced on the full 2,740-case corpus** (TP=1248 FP=552 TN=773 FN=167; SCOREBOARD.md is authoritative — an earlier 47.86% figure here predated the neutral measurement). **≈ Checkmarx (47), just behind Veracode (51), ≫ Fortify (35) & SonarQube (6).** Score via `tsbench sast` (sandbox) or `go run ./cmd/sast-score sg.json expectedresults-1.2.csv` (host semgrep, no sandbox) + per-CWE scorer (`internal/bench/sast.go`). Per-cat: crypto/securecookie/weakrand **100**, hash 69, xss 30, xpathi 28, sqli 24. (Was 38.67% — crypto sat at 0 because semgrep emits **CWE-326** for the DES cases, which the scorer's CWE→category map didn't credit; adding `CWE-326→crypto` recovers all 130 crypto TPs at **0 FP**.) |
