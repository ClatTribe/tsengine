...
func NewHandler(d Deps) http.Handler {
	...
	mux.HandleFunc("POST /v1/assets/{id}/environment", d.auth(d.handleSetAssetEnvironment))        // production | staging | development — unset is gated as production
	...
	mux.HandleFunc("GET /v1/sample-report", d.handleSampleReport)                                         // PUBLIC + UNGATED: the sample VAPT report, generated by the product's own renderer
	...
	mux.HandleFunc("GET /v1/funnel", d.platformAuth(d.handleFunnel))                        // cross-tenant activation funnel (platform-token gated)
	...
	mux.HandleFunc("GET /v1/pentest/scope-suggestion", d.auth(d.handlePentestScopeSuggestion)) // P1 autonomy: propose scope from connected/ownership-verified assets instead of asking a human to retype them
