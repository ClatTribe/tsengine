...
func Propose(f types.Finding, asset platform.Asset, idgen func() string) (platform.Action, bool) {
	...
	case "web_application", "api", "container_image":
		// Respond breadth for the appsec surfaces. These three had NO fix path: they fell to the
		// generic ticket above, which meant the AI Pentester could PROVE an SQLi by exploiting it and
		// the resulting proposal still read "review this finding". A class-correct runbook names the
		// actual fix; ok=false keeps the generic ticket, so an unrecognised class is never given a
		// confident wrong answer. Tier 1 because none of these classes has a live write path — the
		// ticket carries the steps, and a class is promoted to a gated mutation when one lands.
		rt, runbook, ok := appsecFixCatalog(f)
		if !ok {
			return genericTicket(f, asset, idgen), true
		}
		payload := map[string]any{
...
// genericTicket is the honest fallback when no catalog class matches: a reversible tier-1 ticket that
// hands the human the finding and says nothing more. Extracted so the appsec branch can fall back to
// exactly the same shape rather than growing a second, drifting copy.
func genericTicket(f types.Finding, asset platform.Asset, idgen func() string) platform.Action {
	return platform.Action{
		ID: id("act", idgen), TenantID: asset.TenantID, FindingID: f.ID, ConnectionID: asset.ConnectionID,
		Kind: platform.ActFileTicket, Tier: 1, Status: platform.ActProposed,
...
func fixBody(f types.Finding) string {
	// Deliberately does NOT claim a remediation was generated: this body is used both under a runbook
	// (where one was) and alone on the generic path (where none was), and the sentence was false in the
	// second case — a ticket asserting a fix it does not contain.
