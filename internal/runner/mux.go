...
// EmployeeSource lists the tenant's stored HRIS roster (satisfied by the store). Optional: when
// set, the operate runner joins it against the workspace's accounts on every scan, so a leaver
// whose account is still enabled is a finding on the next monitoring pass — not only when someone
// presses Sync.
...
type OperateRunner struct {
	...
	Employees EmployeeSource // optional: the HRIS roster to join against the accounts (internal/hris)
}
...
func (o *OperateRunner) Scan(ctx context.Context, a platform.Asset) ([]types.Finding, error) {
	fs, _, err := o.ScanWithReport(ctx, a)
	return fs, err
}

// ScanWithReport is Scan plus the producers this pass actually observed. "operate" is covered by
// producersForAssetType regardless; "hris" is reported ONLY when the join ran over a non-empty
// roster and a non-empty account list, because a producer that could not run is not authoritative
// about the absence of its findings (detect/coverage.go) — reporting it would let a leaver's
// incident resolve on a pass where the roster was empty.
func (o *OperateRunner) ScanWithReport(ctx context.Context, a platform.Asset) ([]types.Finding, ScanReport, error) {
	if o.Source == nil {
		return nil, ScanReport{}, fmt.Errorf("operate: no workspace source configured")
	}
	ws, err := o.Source.Workspace(ctx, a)
	if err != nil {
		return nil, ScanReport{}, fmt.Errorf("operate: load workspace for %s: %w", a.Target, err)
	}
	...
	findings := operate.Assess(ws, o.Opts)
	var report ScanReport
	if o.Employees != nil {
		// The joiner/leaver join. A roster read error is logged by omission here (the join simply
		// does not run and "hris" is not covered), never turned into an empty roster — an empty
		// roster would read as "no leavers", the opposite of "we could not look".
		if emps, lerr := o.Employees.ListEmployees(ctx, a.TenantID); lerr == nil && len(emps) > 0 {
			hf, rep := hris.Correlate(emps, ws, hris.CorrelateOptions{})
			findings = append(findings, hf...)
			if len(rep.ChecksNotRun) == 0 {
				report.ToolsRan = append(report.ToolsRan, "hris")
			}
		}
	}
