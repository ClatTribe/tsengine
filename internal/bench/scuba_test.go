...
func worstCaseRules(t *testing.T) map[string]bool {
	...
	add(sspm.AssessM365(sspm.M365Tenant{
		...
		RiskyUserNotificationOff:                  true,
		AuthenticatorContextDisabled:              true,
		AuthMethodsMigrationIncomplete:            true,
		ManagedDeviceNotRequiredAuth:              true,
		ManagedDeviceNotRequiredMFARegistration:   true,
		RiskyAIAgentsNotBlocked:                   true,
		SharePointLinkPermissionsNotViewOnly:      true,
		SharePointVerificationCodeReauthDays:      90,
		PhishingResistantMFANotEnforced:           true,
		PhishingResistantMFANotEnforcedPrivileged: true,
		SMTPAuthEnabled:                           true,
		UnmanagedUsersCanInitiateContact:          true,
		InternalUsersNotAutoAdmitted:              true,
		MeetingRecordingUnrestricted:              true,
		AlwaysRecordEvents:                        true,
		UnmanagedUserContactAllowed:               true,
		AppPasswordAdditionAllowed:                true,
		AppPasswordLifetimeDays:                   730,
		AppCertificateLifetimeDays:                1095,
		GuestDirectoryAccessUnrestricted:          true,
		AnyUserCanInviteGuests:                    true,
		GuestInvitesAnyDomain:                     true,
		DeviceCodeAuthAllowed:                     true,
		SecurityLogsNotExported:                   true,
		PrivilegedAssignmentNoAlert:               true,
		OtherPrivilegedActivationNoAlert:          true,
		PrivilegedAccountsNotCloudOnly:            true,
		StandingGlobalAdmins:                      2,
		PrivilegedRolesOutsidePIM:                 true,
		GlobalAdminActivationNoApproval:           true,
		GlobalAdminActivationNoAlert:              true,
	...
	add(sspm.AssessGoogleWorkspace(sspm.GWorkspaceTenant{
		...
		FormsAcceptExternalResponses:          true,
		FormsSubmitExternally:                 true,
		SharedDriveCreationOverridable:        true,
		NonMembersCannotBeAddedToFiles:        true,
		PostSSOVerificationOff:                true,
		PostSSOVerificationOffOther:           true,
		UnassessedServicesEnabled:             true,
		EarlyAccessAppsEnabled:                true,
		LookerStudioExternalShare:             true,
		PinpointDriveAccess:                   true,
		RecoveryInfoAllowed:                   true,
		AccountConflictUnmanaged:              true,
		PhishingResistant2SVNotEnforced:       true,
		UnconfiguredThirdPartyAppsAllowed:     true,
		DriveSDKEnabled:                       true,
		POPIMAPEnabled:                        true,
		SecondaryCalendarExternalSharing:      true,
		CalendarInteropEnabled:                true,
		CalendarInteropBasicAuth:              true,
		AppointmentPaymentsEnabled:            true,
		SimilarDomainSpoofProtectionOff:       true,
		EmployeeNameSpoofProtectionOff:        true,
		GroupsSpoofProtectionOff:              true,
		LinkAutoApplyOff:                      true,
		SpoofAutoApplyOff:                     true,
		PreDeliveryScanningOff:                true,
		UserEmailUploadsEnabled:               true,
		ExternalOutboundGateway:               true,
		SpamBypassAllSenders:                  true,
		MailDelegationEnabled:                 true,
		DeviceTrustAllowed:                    true,
		SessionNeverExpires:                   true,
		TwoSVEnrollmentPeriodDays:             30,
		TakeoutEnabled:                        true,
		UserConsentLowRiskScopes:              true,
		UnconfiguredInternalAppsTrusted:       true,
		AdminAccountsNotCloudOnly:             true,
		DriveExternalShareWarningsDisabled:    true,
		DriveOutOfDomainWarningsDisabled:      true,
		DriveSecurityUpdateNotApplied:         true,
		DriveRansomwareMonitoringDisabled:     true,
		DriveNonGoogleAccountSharing:          true,
		DriveExternalSharedDriveUpload:        true,
