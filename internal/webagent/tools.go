...
func tools() []toolDef {
	return []toolDef{
		...
		{"record_finding", "record_finding(route, class, evidence[], severity, rationale) — commit a vuln. class ∈ sqli|blind_sqli|xss|dom_xss|stored_xss|ssti|ssrf|idor|open_redirect|path_traversal|lfi|xxe|command_injection|rce|default_credentials. REJECTED unless a cited turn carries the deterministic indicator for that class (e.g. idor/bola needs bola_confirmed — run bola_probe(url,attacker_cookie,victim_cookie,marker) first; ssti needs ssti_eval — a {{A*B}} probe with MULTI-DIGIT factors so the product is >=4 digits, e.g. {{1234*1234}}, NOT the textbook {{7*7}}: a tiny product like 49 is too collision-prone to ground, so it never fires the indicator even though the page shows it; the engine returns the computed product; xxe needs file_disclosure — the external-entity'd file content in the response; ssrf needs oob_interaction — mint an oob_url(), put it in the vulnerable fetch/url param, then oob_check(token): a recorded callback PROVES the target made the request and emits a citable turn to cite here).", tRecord},
