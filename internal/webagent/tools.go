...
var requiredIndicator = map[string][]string{
	...
	// NoSQL (MongoDB) injection (nosqli.go): a Mongo-operator payload ($ne/$regex/$where …) flips access —
	// a marker absent with a benign value and present under the operator injection (and not echoed).
	"nosqli": {"nosqli_confirmed"}, "nosql_injection": {"nosqli_confirmed"}, "mongodb_injection": {"nosqli_confirmed"},
	// broken session management (logout.go): a session that survives its own logout — the private marker
	// is readable before logout and STILL readable with the same cookie after. The account-takeover
	// primitive (a stolen token stays valid). Session FIXATION is deliberately excluded — its predicate
	// (pre-auth vs post-auth session-id) is not always observable, and a shaky one is worse than a gap.
	"broken_session_management": {"session_invalidation_failed"}, "session_fixation": {"session_invalidation_failed"}, "logout_invalidation": {"session_invalidation_failed"},
}
...
func tools() []toolDef {
	return []toolDef{
		...
		{"nosqli_probe", "nosqli_probe(method, url, marker [, control_body, inject_body, inject_url, cookie]) — GROUND a NoSQL (MongoDB) INJECTION: a login/search where a client string reaches a Mongo query and an OPERATOR payload bypasses it (auth bypass or filter bypass). Send a CONTROL request (benign value — wrong creds, a normal search) and an INJECTION request that adds a MongoDB operator: a JSON body like {\"username\":{\"$ne\":null},\"password\":{\"$ne\":null}} or {\"$regex\":\".*\"}, or the query-string form ?username[$ne]=&password[$ne]= (put it in inject_body for POST, or inject_url for a query-string). marker = the SERVER-ORIGINATED content the bypass exposes (a post-login page, a victim's record, a flag) — NOT a value you send. It sets nosqli_confirmed ONLY when the marker is ABSENT in the control response, PRESENT in the injection response, NOT sent by you (echo guard), AND the injection actually carries a Mongo operator — so a generic value flip or a reflection can't false-positive. Then cite the turn in record_finding(class=nosqli).", tNoSQLiProbe},
		...
		{"logout_probe", "logout_probe(protected_url, logout_url, session_cookie, marker) — GROUND a BROKEN-SESSION-MANAGEMENT finding: a session that survives its own logout. protected_url = an authenticated page rendering session-PRIVATE data; logout_url = the app's logout endpoint; session_cookie = the logged-in session; marker = a private datum you can see on protected_url while logged in (their email/account-no — NOT nav/chrome). It reads protected_url (baseline: proves the session HAD access), calls logout with the same cookie, then re-reads protected_url. It sets session_invalidation_failed ONLY when the marker was PRESENT before AND STILL present after logout (the token outlived its logout) — FP-free (a mere 200 on a logged-out page proves nothing; the private-marker differential does). Cite that turn in record_finding(class=broken_session_management). This is the account-takeover primitive: a stolen token stays valid past logout.", tLogoutProbe},
