...
func tools() []toolDef {
	return []toolDef{
		...
		{"dispatch_oss", "dispatch_oss(tool, args) — hand a SPECIALIZED job to an OSS specialist instead of doing it by hand: sqlmap (automated blind/error SQL-injection EXTRACTION when manual is infeasible), wpscan (WordPress CVEs), nuclei (known-CVE templates), ffuf (large-scale fuzzing), hydra (login brute-force). args is that tool's own arguments (e.g. sqlmap: {\"url\":\"...\",\"data\":\"...\"}). Use this — don't reimplement char-by-char blind extraction or a CVE exploit yourself.", tDispatchOSS},
		{"ssh_exec", "ssh_exec(user, password?, private_key?, command, host?, port?) — the EXPLOIT step for a LEAKED CREDENTIAL. When an info-disclosure / source leak / config dump hands you an SSH username + password (or private key) — the flag or next hop is usually behind SSH, not HTTP — connect and run ONE command to read it (e.g. command=\"cat /home/<user>/FLAG.txt\"). host defaults to the target's host (SSH is normally on the same box); port defaults to 22. Returns the command's real output (grounded). Scope-guarded to the authorized target. This turns \"I found creds\" into a proven lateral-movement finding — don't stop at discovering the credential.", tSSHExec},
