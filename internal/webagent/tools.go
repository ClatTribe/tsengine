...
func tools() []toolDef {
	return []toolDef{
		...
		{"record_finding", "record_finding(route, class, evidence[], severity, rationale) — commit a vuln. class ∈ sqli|blind_sqli|xss|dom_xss|stored_xss|ssti|open_redirect|path_traversal|lfi|command_injection|rce|default_credentials. REJECTED unless a cited turn carries the deterministic indicator for that class (e.g. ssti needs ssti_eval — a {{A*B}} probe whose product the engine returned).", tRecord},
