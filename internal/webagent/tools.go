...
var requiredIndicator = map[string][]string{
	...
	// CORS misconfiguration (cors.go): the response reflects an arbitrary attacker Origin (or "null") in
	// Access-Control-Allow-Origin WITH Access-Control-Allow-Credentials:true — cross-origin credentialed
	// read. ACAO:"*" is NOT grounded (browser-forbidden with credentials → not exploitable).
	"cors": {"cors_confirmed"}, "cors_misconfiguration": {"cors_confirmed"}, "cross_origin_resource_sharing": {"cors_confirmed"},
}
...
func tools() []toolDef {
	return []toolDef{
		...
		{"cors_probe", "cors_probe(url [, origin, cookie]) — GROUND a CORS MISCONFIGURATION: a server that reflects an arbitrary attacker Origin (or 'null') into Access-Control-Allow-Origin WITH Access-Control-Allow-Credentials:true, letting a malicious cross-origin site read this app's credentialed responses (session-scoped data theft). Pass the app's own session cookie so the response is credentialed; origin defaults to an arbitrary canary. It sets cors_confirmed ONLY on the reflected-arbitrary-origin (or 'null') + credentials-true differential — an ACAO of '*' is NOT grounded (browser-forbidden with credentials → not exploitable), and a static trusted origin won't reflect yours, so neither false-positives. Then cite the turn in record_finding(class=cors).", tCORSProbe},
