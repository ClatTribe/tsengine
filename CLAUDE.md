...
**Runtime protection — the /Protect pillar (`internal/protect`, [docs/adr/0013-protect-managed-zen.md](docs/adr/0013-protect-managed-zen.md)):** tsengine does NOT block in production (§13 — a RASP is exactly what we don't build in-house). Instead we **wrap OSS Zen** (`@aikido/firewall`, the customer's installed in-app sensor that blocks at the sink) and are the management+telemetry layer. `protect.Compute(events, since, topN)` rolls the ingested `platform.RuntimeEvent`s (`POST /v1/runtime/events`, already correlated by `crossdetect.AnnotateRuntime`) into a runtime-protection posture — apps reporting, attacks blocked vs monitor-only, block-rate, top attack kinds + endpoints. `GET /v1/protect`. Grounded (§10): no events → `active:false` ("no runtime signal yet", never "protected"); monitor-only → active with block-rate 0 (honest). The `/protect` marketing page + the in-app `/runtime` posture view + the Security-nav entry are BUILT; the managed Zen DEPLOY (config distribution to the customer sensor) is the Phase-1 follow-on.
