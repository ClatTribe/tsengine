...
| **AI Compliance Engineer** | `internal/grcexport` | **Push-to-Drata** (compliance-platform partnership): writes the engine's control posture INTO the customer's Drata as Custom-Connection RECORDS their own tests evaluate. The integration shape is Drata's and it is the honest one — we push RECORDS (met/gap per control + finding count + evidence link) and the pass/fail TEST is authored in Drata's dashboard and mapped to a control THERE, so the engine never tells Drata a control is met; a control the engine has not assessed is simply not a record (§10). A sync is one SESSION completed atomically (full-state replace), and an EMPTY set is refused rather than pushed — completing an empty session erases Drata's previous batch and reads as "TensorShield reports nothing", which is not "we assessed nothing". `GET/PUT /v1/settings/drata` (API key SEALED via `d.Vault`, workspace id; changing the key resets the stored connection ids) + `POST /v1/settings/drata/sync`; the connection is created once (`EnsureConnection`) and its ids stored for reuse. Vanta's equivalent (a private integration that PUTs custom resources, Custom Tests dashboard-authored) is the same shape and the documented follow-on. |
| `internal/grc` | 22-framework evidence | OpenCRE / SCF / CSA CCM (crosswalk corroboration) |
...
**Adding a framework touches SEVEN places, not the four named above.** Three further guards caught the shortfall when DORA and UK Cyber Essentials landed: `grc.complianceControls` (the PER-ASSET view — a fifth mirror, whose own test says omitting it is "the exact defect that left certin/rbi/sebi invisible"), the per-framework DEPTH FLOOR in `grc/coverage_depth_test.go` (a new framework must record one or the corpus-shrink guard silently stops covering it), and `ComplianceCorpusVersion` + its recorded digest (bumped IN THE SAME CHANGE, or two different corpora are indistinguishable inside a pinned evidence pack). Trust the guards over this list.

**DORA and UK Cyber Essentials (added 2026-09-05)** follow the India frameworks' DERIVATION rule — every ref traces to a CWE's already-curated NIST 800-53 family rather than a fresh guess. DORA is mapped DELIBERATELY NARROW to the ICT-risk articles a finding can evidence (Art. 8 identification, Art. 9 protection and prevention, Art. 10 detection, Art. 24 testing); its governance, incident-REPORTING and contractual articles are procedural duties with no per-finding nexus, and mapping them would be the false-compliance this section forbids — the same line CERT-In is drawn on. Refs stop at the ARTICLE and do not name sub-letters, because precision that cannot be verified is false precision in an audit artifact. Cyber Essentials cites 4 of its 5 controls: no CWE in the crosswalk carries the malware-protection (SI-3) nexus, so that control is honestly uncited rather than padded.
...
| Mutations are human-gated (HITL) | The agent opens a PR/ticket and pauses for a human approval; its own cloud access stays read-only (`cloudsafety.Guard` + scoped STS). **The LIVE INVENTORY READ is scoped the same way**: `awsfetch.assumeRoleConfig` (every S3/IAM/EC2/Lambda/RDS/CloudTrail lister) assumes the customer role WITH `cloudsafety.SessionPolicy()` as an inline STS session policy, so the read is the INTERSECTION of the role and the read-only cap and cannot mutate or read data contents even if the customer over-granted the role. This was documented as done and was NOT applied on the fetch path until it was wired here -- the claim held only for the agent live-validation calls; a session policy denies only what it names, and the listers do metadata reads (Describe/List/Get-policy) it does not deny. |
...
**A finding the per-pass SYNC paths store must ALSO be folded into the compliance posture.** The
per-asset path gets that for free (`runner.processFinding` → `grc.Apply`); `syncDevices` and
`assessTraining` do not, and without `runner.foldPosture` they stored the finding and stopped — so
the issues list showed a gap while the compliance posture read clean, and the two doors for the SAME
assessor disagreed (the posted-inventory ingest folds; the live sync did not). A control is opened
only because a real finding CITES it (§18.2 inv. 5), so a finding nobody folds is evidence the
control layer never sees. Mutation-verified in `internal/runner`.
...
| `internal/prbot` | **repository** Â· PR-inline review bot (vs Aikido/Snyk) | `Build(findings, changedFiles, blockAt)` â inline comments **only on PR-changed lines** + a check-run `success/neutral/failure`. CI entry point `POST /v1/ci/pr-check` (+ `docs/ci/github-action.yml`) runs the merge-gate from any CI via the check's exit code. **The live post is BUILT** (`prbot.GitHubPoster` + `internal/connector/ghapp`): a GitHub App (`GITHUB_APP_ID` + `GITHUB_APP_PRIVATE_KEY`, permissions `checks: write` + `pull_requests: write`) mints an installation token — only an App can own a check-run, so the OAuth connection could never post the merge gate, which is why `Submit` was only ever called with a nil Poster — and the pr-check posts the check-run then the review when the request names the PR (`repository`/`pull_number`/`head_sha`, which the action now sends). Each workspace records WHERE the App is installed (installation id on the GitHub connection's Config, via `PUT /v1/settings/pr-bot`). The response carries `posted` + `not_posted_reason`: three facts with three owners (operator's App, customer's install, customer's connection), and the reason names the first missing one because "the bot did not comment" otherwise sends the reader to the wrong fix. `posted` is true only when GitHub accepted every call that was due — a review the developer cannot see is no review, whatever the API returned. Settings renders the server's status, never a client-side guess (`uicheck`). |
...
| `internal/identitythreat` | **identity** Â· real-time ITDR (vs Nudge/Push) | `Detect(events)` rules: impossible_travel, privileged_grant, mfa_removed, password_spray, distributed_spray, mfa_fatigue, concurrent_session (two logins from different IPs within a tight window → session-token reuse, distinct from travel which needs different countries), mfa_removed_then_access (MFA disabled then a login from a new IP → the account-takeover sequence) â LLM-free, grounded. **Live IdP audit-log ingestion is BUILT** (`internal/identitylog`): Okta System Log, Entra sign-ins + directory audits (Graph), Google Admin Reports (login + admin apps), each read through the ALREADY-onboarded connection's token (read scopes only) and normalised into `identitythreat.Event`. Before this the nine rules had no source but a customer-built pusher to `POST /v1/identity/events`, so they were execution-proven in tests and silent in production. Runs EVERY monitoring pass (`runner.SyncIdentityLogs`, cursor per provider on `Tenant.IdentityLogCursors`, 24h window with a 1h overlap, dedup by provider event id) and on demand (`POST /v1/identity/sync`). Honesty: unmapped provider event types are COUNTED, what a provider's log cannot say is DECLARED (`ChecksNotRun` — Google carries no country so impossible-travel is not evaluable; Graph records strong-auth outcomes not pushes so MFA-fatigue is not), a non-log 2xx (an HTML sign-in page behind an expired token) is an ERROR not an empty window, a failed read never advances the cursor and never counts as covered. |
...
| cloud_account · GCP privesc (EXTERNAL) | `internal/bench/gcpprivesc.go` (`RHINO_GCP_CATALOGUE`) | recall over RhinoSecurityLabs' 23-method catalogue | **Rhino's published research — the same one `internal/gcpiam/privesc.go` already cited.** Scored 65.2% on first run, almost exactly AWS's 64.5%: two independent keys each said ~two thirds, and no internal number ever did. **RECALL ONLY** — GCP has no published FP control set, so read it one-sided. **NOW 23/23 (100%), AND THE 65.2% IS THE WEAKEST NUMBER ON THIS PAGE — it was measured in the SAME commit (#1270) that added the eight techniques closing it, which is exactly what §14.2 rule 5 forbids, so it is unfalsifiable and recorded here as history rather than as a result.** The two AWS keys were taken before their gaps were closed and are worth more for it. Re-measured 2026-09-13: 23 of Rhino's 23 methods detected, the parse verified complete against the catalogue source (23 top-level keys), and the scorer verified to still DISCRIMINATE — dropping a technique drops recall by exactly one and names the method, so the 100% is a result and not an instrument that stopped matching. Note what cannot be re-measured: with no FP control set there is no number here that can go DOWN, so an exhausted GCP recall says less than an exhausted AWS one. **The techniques are now proven to reach the PRODUCT** (`gcpinventory.TestBuild_RhinoCataloguedTechniquesProduceAPrivescEdge` drives all 18 distinct ones through the real ingest) — that guard did not exist, so the published recall rested entirely on direct calls to the catalogue while nothing asserted a customer's project would produce an edge |
| cloud_account · CloudGoat as PUBLISHED (EXTERNAL) | `internal/bench/cloudgoat.go` (`CLOUDGOAT_DIR`) | recall over the IAM-decidable subset of Rhino's CloudGoat scenarios | **A THIRD independent key. Measured 57.1% on FIRST RUN (4/7 IAM-decidable, 28 scenarios read) — the third to land near two thirds, against ~100% on every in-house bench; now 100% after the three misses it named were closed.** DISTINCT from `tsbench cloud-engine --cloudgoat`, which replays TWO scenarios we TRANSCRIBED — that is our reading of a lab; this reads the corpus as Rhino published it. The denominator is the subset Rhino's OWN naming/summary calls a privilege escalation, because CloudGoat is broader than IAM (SSRF, RCE, Cognito) and a policy-only read MUST miss those — a miss there is a correct refusal, so the excluded scenarios are LISTED with Rhino's documented route rather than averaged away. Two refusals make the number mean something, both mutation-verified: a document granting `*` is ADMIN not an escalation (without it ONE wildcard doc credits all 30 techniques and recall reads 100%), and `aws_iam_role`'s inline document is the TRUST policy — who may assume the role, granting the principal nothing — so mining it would manufacture escalations. **The extractor was FIXED BEFORE the number was recorded**: it ended with a CONSUMED delimiter (RE2 has no lookahead), silently dropping every OTHER adjacent policy — three adjacent `aws_iam_policy` blocks yielded two. That bias runs one way, toward a recall that is too LOW, so it was an instrument bug to fix rather than a gap to report; after the fix the per-scenario document counts rose (`iam_privesc_by_ec2` 2→3) and the recall held at 57.1%. **THE THREE MISSES ARE NOW CLOSED IN A SEPARATE CHANGE AND THE RECALL READS 100% — so this corpus is NO LONGER HELD OUT, and 57.1% is the number that meant something.** Each miss was one absent primitive, not tuning: `PassRoleToNewECSTask` (a task runs as the role its definition names — the ECS twin of `PassRoleToNewLambda`), `PassRoleToNewGlueJob` + `UpdateExistingGlueJob` (the catalogue knew only `glue:CreateDevEndpoint`, the method Rhino's original catalogue names, while dev endpoints are deprecated and JOBS are how Glue code is run today — the door we watched was the one closing), and `EC2UserDataModification` (`ModifyInstanceAttribute` + `StopInstances` + `StartInstances`: AWS refuses a userData change on a running instance, so the escalation IS the stop-modify-start cycle and requiring only the modify would report a path nobody can walk). Recall can only rise when detections are added, which is why the number to read now is the FP half — and it is BishopFox's, since CloudGoat publishes no control set: **0/5 false positives and 0/4 false negatives, unchanged by these four additions**, re-measured rather than assumed. Each is pinned in BOTH directions (`internal/cloudiam/privesc_cloudgoat_test.go`), because only the negative half gives the requirement lists meaning: `ecs:RegisterTaskDefinition` is held by anyone who deploys a container and `ec2:ModifyInstanceAttribute` by most infrastructure roles alive, so a technique firing on one action alone would report an escalation for most principals in a real account. And the primitives are proven to reach the PRODUCT graph, not only the bench (`awsinventory.TestBuild_CloudGoatPrimitivesProduceAPrivescEdge` drives the real ingest) — a catalogue entry proven only through `DetectPrivesc` is the built-but-unwired shape, where the bench reads 100% while a customer posting that account gets no edge |
...
| `internal/auditreview` | **the reviewer's decision, recorded — the step between a scan and a signed certificate.** Built for the CERT-In-empanelled audit firm motion (§18.5 `msp`): their cost is not the scan, it is the qualified reviewer deciding, finding by finding, what goes into a document they put their name on. **THE COMPRESSION IS ATTENTION, NOT AUTOMATION** — auto-including above a confidence threshold would be easy and wrong, because the reviewer is the one signing and a tool that quietly made that call would manufacture exactly the false confidence §10 prevents. What it does instead is tell them where their judgement is load-bearing: a finding the agent EXPLOITED with a captured proof is a glance, a `pattern_match` is their name on the scanner's word. `Load` states the split and the ORDERING puts the expensive minutes at the top, so an hour goes to the four findings that need it rather than spread evenly over twelve. Rebuilt from CURRENT findings each read with decisions merged back on (the `recertify` shape) because the tender flow re-tests between draft and final report — a frozen review would send a reviewer to sign off an application whose latest scan found something nobody has seen; a new finding REOPENS the review, and decisions survive a re-scan by keying on `crossdetect.DedupKey` rather than the finding id. **SEVEN REFUSALS, all mutation-verified**, because on this artifact a wrong answer is forwarded to a government buyer who cannot see what it rested on: an unattributed decision is a log line; exclusion and reclassification REQUIRE a reason (an exclusion with no reason is indistinguishable from an oversight, and if the app is later compromised through one, that sentence is what stands between the auditor and negligence); an EMPTY review is not complete (zero findings is as likely a scan that never ran); no certificate while anything is undecided; none while a serious finding is open and unretested; the statement always carries its limits (what was excluded, what remains open, **what was NOT tested** from the scan's own `asset.CoverageRulePrefix` gaps, and that it is not a guarantee); and a decision about a finding not in scope is REFUSED rather than stored, or it would sit uncounted forever (the off-roster shape). An exclusion NEVER deletes. Reclassification is named for either direction — a reviewer who knows the app is internet-facing may legitimately RAISE a medium — but a LOWERING is recorded as such, taken from the engine's own severity, since that is the direction that makes a report look better. The certificate STATEMENT is generated rather than typed: a free-text box is where "no vulnerabilities were found" gets written about an audit that excluded four findings and never tested the API. `Certify` returns EVERY blocker, not the first, and they ride on the READ as well as the issue attempt so a reviewer clears them while working. Auditor FIRM + CAPACITY resolve from the practitioner roster, never the request, so a certificate cannot claim an independent firm signed it because somebody typed one in. `GET /v1/audit-review` · `POST /v1/audit-review/disposition` · `POST /v1/audit-review/certificate` (409 + blockers, not 400 — the request is fine, the audit is not ready) + the `/audit-signoff` tab, guarded by `uicheck` including a ban on any bulk-approve control. **The DOCUMENT half (`certificate_render.go`, `GET /v1/audit-review/certificate?format=html\|md\|json`)**: two more blockers first — `scope_untested` / `scope_partial`, read from the SAME `untestedScope`/`partiallyAssessedScope` the VAPT report uses, because a finding list scoped to a target says nothing about whether it was scanned (findings arrive by import; a scan missing half its tools still lands what survived) and a certificate is a stronger claim than the "Clear" rating the report already refuses on them. Then an ed25519 attestation on the evidence-pack scheme (`Attestation` is an ALIAS of `grc.Attestation` — one format, two names; `Sign` refuses anything `Certify` did not issue), a content-derived id (`STH-…` from target + auditor + day), a twelve-month ceiling voided earlier by any change to the application, and print-ready HTML / Markdown that quote the signed statement VERBATIM (a renderer that paraphrased it would be a second author of a signed document). The GET is the POST's twin over the same review and options, so the two cannot disagree about whether a certificate exists; it refuses 409 with blockers, 501 with no signing key (never an unsigned artifact), and ledger-records every served document with its digest. Download links on the desk appear only once certifiable. CERT-In empanelment is the auditing FIRM's credential and the certificate asserts nothing about it. |
| **the per-application SKU** (`platform.PlanAudit` + `platform.AuditOrder` + `internal/platformapi/auditorders.go`) | **the commercial half of the audit desk — one application, one price, paid on acceptance of its certificate, never in advance.** Nine public-sector VAPT tenders read in 2026-09 all set that term ("100% after acceptance of the final report, no advance") and priced a web-app audit + re-test + certificate at ≈₹40–60k per application; `AuditListPriceINR` (49,999) sits in that band and is MIRRORED on the pricing page — change both. `auditreview` is the WORK; `AuditOrder` records only the MONEY, and its status machine IS the term: `open → certified → accepted → invoiced`, with `AmountDueINR` COMPUTED from status (zero until accepted) rather than stored, so a page an operator invoices from cannot show money owed on work the buyer has not accepted — `uicheck` pins that the desk's order panel renders the server's `amount_due_inr` and never derives a figure of its own. Three doors, three actors: the firm opens an order (`POST /v1/audit-orders`, list or agreed price, one OPEN order per application — a second is a double-click, not a sale), the BUYER's named human accepts (`POST /v1/audit-orders/{id}/accept`, refused until a certificate exists — accepting undelivered work is the advance the terms forbid, and an unnamed acceptance is refused because it is the payment event), the OPERATOR invoices (`POST /v1/tenants/{t}/audit-orders/{id}/invoice`, platform token, refused before acceptance). BOTH certificate doors call `markAuditOrdersCertified` so the order cannot depend on which button was pressed. The tier itself: `PlanAudit` (aliases `per-app`, `per-application`), AI + pentester ON (the fee covers the model cost for that application), continuous monitoring OFF (a point-in-time engagement with a re-test, not a heartbeat), and the asset cap is the number of applications PURCHASED (`Tenant.AuditApplications`, set with the plan by the operator) — read through `EntitlementsFor(tenant)` because a plan string cannot carry a count; the bare `Entitlements("audit")` returns MaxAssets 0 so a caller that forgets fails CLOSED rather than scanning unpaid. Store entity `AuditOrder` (6-place, isolation asserted — one firm's price list is a competitor's pipeline). |
...
| `internal/cloudcdr` | **cloud DETECTION-AND-RESPONSE — the control-plane EVENT signal, and it is now POLLED, not only posted.** The rules (public-resource exposure, security group opened to the world, root console login, IAM privilege escalation / new credentials, audit logging stopped) existed with ONE source: `POST /v1/cloud/events`, a forwarder the CUSTOMER had to build. So a root login at 2am opened an incident only if somebody else's pipeline told us — the same execution-proven-in-tests / silent-in-production shape `identitythreat` had before `identitylog`. `awsfetch.CloudTrailLister` reads the 90-day event history CloudTrail keeps for EVERY account (cloudtrail:LookupEvents — a READ in ReadOnlyAccess, through the same connected role the inventory fetch assumes; no trail, no S3 bucket, no EventBridge rule to configure), `cloudcdr.FromCloudTrail` is the ONE translation of a raw record into the rules' `Event` (root identity → actor "root"; the request parameters serialised as `Detail` because the VALUES that make an action dangerous — `0.0.0.0/0`, `AllUsers`, `AdministratorAccess` — live there; an unparseable record is DROPPED AND COUNTED, never guessed at), and `runner.SyncCloudEvents` runs it EVERY monitoring pass (cursor per connection on `Tenant.CloudEventCursors`, 24h first window, 1h overlap because the API lags ~15 minutes) and on demand (`POST /v1/cloud/events/sync`, the Settings "Read CloudTrail" button on the AWS row, `uicheck`-guarded). Both doors run the same rules and the same L1.5 chain, so they cannot disagree. Honesty: the API returns NEWEST first and a busy account can outrun one read, so a read that hits its bound still advances the cursor (or it would re-read the same newest records forever and never reach the rest) and NAMES the older span it did not examine in `Unread` — "0 threats" over a partial window is a claim about the read, not the account; a failed read is named in `Failed`, never an empty window, and never advances the cursor or counts as covered. Honest floor, stated: management events only, one region per lister, ~15 minutes behind, and CloudTrail's event history is not a trail — a customer with an organisation trail gets data events and multi-region; this gets everyone something without a pipeline. |
| `internal/clouddrift` | **continuous CONFIG-SNAPSHOT drift detection** — the third drift signal, complementing `cloudcdr` (audit-log-event drift) + `detect` (finding-diff drift). `Diff(prev, cur *cloudgraph.Snapshot)` compares two cloud inventory snapshots over time and emits grounded change-control findings for the security-relevant config changes: `resource-became-public`, `principal-became-privileged`, `new-privileged-principal`, `new-public-resource`, `new-internet-exposure` (a new internet→resource reach edge), `new-privilege-escalation`, `new-lateral-movement` (new secret_access edge). The SOC2/CIS change-control "detect unauthorized change to the environment" signal WITHOUT the audit-log stream. Deterministic + LLM-free + grounded (§10): each finding cites the changed node/edge + before→after, carries the change-control compliance nexus (SOC2 CC8.1, NIST CM-3/CM-6, CIS 4.2/8.5, ISO A.8.32), and an unchanged pair / nil baseline (first observation) yields ZERO findings. Live driver: `POST /v1/cloud/drift` (prev+cur inventory → findings into the same store → issues/incidents/grc/hitl). **Continuous DIFF-ON-INGEST is now wired** (`cloudinventory.go`): re-POSTing to `POST /v1/cloud/inventory` diffs the fresh snapshot against the tenant's STORED baseline BEFORE overwriting it → automatic drift findings on every re-ingest (the "connect once, detect change" promise), no separate `/v1/cloud/drift` call. Both paths share `persistDriftFindings` (enrich → store → GRC → incident) so they never diverge; grounded §10 (first ingest / unchanged re-ingest yield 0). **The live scheduled fetch exists** (`runner.CloudSyncer` → `SyncCloudInventory` every monitoring pass, reading S3 + IAM + EC2/SGs **+ Lambda + RDS** through the connected role — `awsfetch.LambdaLister` carries each function's execution role and whether a function URL exposes it with no auth (that URL IS the internet edge; an IAM-auth URL or an unread URL config asserts nothing), `awsfetch.RDSLister` carries the public endpoint, port, security groups, encryption and the customer's declared-sensitivity tag (never the engine or name), and the graph applies the instance rule to a database — a public endpoint is not reachability, the security group on the listening port decides. `RawAWS` had NO field for either, so a live account carried no serverless compute and no relational store at all. The same change closes a live-path hole in EC2: the lister never recorded the instance profile, so no live instance ever got a `runs_as` edge — the step every host-to-data path runs through; profiles are now resolved to roles through IAM's own listing (`InstanceProfileReader`), never by rewriting the ARN, and an unresolvable read is NAMED in `Skipped["instance-profiles"]`. Still unread on the live path and said so by `Coverage()`: EKS, KMS, CloudTrail trail CONFIGURATION/Config (its EVENTS are polled — see `internal/cloudcdr`), IAM groups, Organizations SCPs) **and now runs the SAME assessments as the posted door**: the CI/federated-identity analysers (ghoidc, samltrust) and `connector.CoverAWS` notes ran only in `handleIngestAWSInventory`, so a role any GitHub repository could assume was found when its trust policy was POSTED and missed when it was READ — and the read is the path a connected customer actually uses. Fifth instance of the two-doors-disagree shape. **The cloud-account SCAN (prowler/scoutsuite) is also tenant-scoped now** (`cmd/platform/cloudcreds.go`): the sandbox dispatcher forwarded the platform's OWN `AWS_*` environment into every cloud scan while the connection's `role_arn` sat unread in the asset Meta, so CIS posture for a connected account ran as the OPERATOR against whatever account those variables reached. An asset carrying a role is scanned AS that role through `awsfetch.AssumeRoleConfig` (same region default, tenant id as external-id guard), and an unassumable role FAILS the scan (degraded pass) rather than falling back to the operator — a scan that quietly runs as a different principal is the defect itself. Role-less assets (operator-credentialed, GCP/Azure) keep the operator env. |
...
| `internal/training` | **security-awareness training — the PEOPLE evidence the product could not produce.** SOC 2 CC1.4/CC2.2, ISO A.6.3, PCI 12.6 and HIPAA 164.308(a)(5) all ask whether the people who work here were trained; the engine could evidence a great deal about machines and, before this, nothing about people beyond policy acknowledgements and (since the HRIS join) a roster. **THE REFUSAL THAT SHAPES THE PACKAGE**: we are not a content company, and a completion record for a module nobody read is compliance theatre — so a completion carries the EVIDENCE TIER that produced it and the two are NEVER merged. `delivered` = the content was rendered in THIS product and the person confirmed it (the strongest claim software can make without proctoring an exam, which is why the status is never "passed"); `attested_external` = a named human recorded that it happened elsewhere, NAMING the provider. **No combined percentage is emitted at all** — one figure spanning both would RISE as a customer asserted more and evidenced less, the defect `ctoreadiness` and the questionnaire each refuse by name; a test asserts structurally that `Summary` never grows a Percent/Rate/Score field. Three further refusals, each mutation-verified: EXPIRED is not OUTSTANDING (an annual module met 400 days ago is due again, not untouched — a refresher gap and an onboarding gap are chased differently); **no roster means no denominator** (`NoRoster` — "nothing outstanding" over nobody is not a trained workforce, and the detail line says so in those words); and a completion recorded against somebody the roster does not know is NAMED (`OffRoster`) rather than silently uncounted, or whoever entered it watches the summary not move and assumes it landed. The curriculum is five modules with REAL content in the repo — that content is the only thing making `delivered` mean anything — five rather than fifteen, since a curriculum long enough to feel thorough is one people click through. `Assess` reports ONE finding PER PERSON (five modules × forty people is two hundred rows, and an issues list that size teaches people to scroll past it), keyed on the person so the row closes when they finish, at MEDIUM severity because it is a control gap and paging someone at night over a training reminder is how an alerting channel dies. Store: `platform.TrainingCompletion`, APPEND-ONLY and idempotent per day (three clicks make one record; last year's completion survives, because "trained every year since 2024" is what an auditor asks and current state cannot answer). **TWO WRITE PATHS, deliberately not one**: `POST /v1/training/complete` takes the subject FROM THE SESSION and can never be told who it is for (delivered asserts WE showed it to THAT person), and `POST /v1/training/record` forces the attested tier, requires the provider, and honours the real DATE while refusing a future one. `GET /v1/training` + the `/training` Compliance tab, with five `uicheck` guards. Assessed on the CLOCK by `runner.assessTraining` each pass, because a completion lapses on its anniversary with no event at all; an EMPTY roster returns false and is not stamped as covered. **Questionnaire HR-2 was PROMOTED from attested to observed on the strength of this** — its old reason ("training completion lives in an HR or LMS system we do not read") had become false — and the promotion is safe only because both halves are grounded: the `training` source is stamped only when a roster exists, and anyone who owes training raises a finding citing CC1.4, so a roster that has done nothing reads `In progress`, never `Yes`. A test asserts all three states. |
| **vendor REGISTER** (`platform.Vendor` + `internal/platformapi/vendors.go`) | **the durable third-party inventory — not the findings it raises.** `internal/tprm` assessed a vendor list that lived only inside the body of `POST /v1/tprm/ingest`: the FINDINGS persisted and the PORTFOLIO did not, so the product could say which suppliers FAILED a check and could not answer "who are our vendors" at all — the same defect the Trust Center's sub-processor note describes from the other side (a derived list names the failures and omits every well-managed vendor), and a register is precisely the artifact SOC 2 CC9.2 / GDPR Art. 28 / PCI 12.8 ask for. `tprm.Vendor` and `tprm.DataAccess` are now ALIASES of the stored `platform.Vendor` / `platform.VendorDataAccess` (one type, two names — a mirrored struct would drift into an audit artifact), which is why `handlesPersonalData`/`hasCert` became functions: Go forbids methods on an aliased foreign type. **ONE REGISTER, TWO DOORS** — the posted inventory and the register editor both write through `saveVendors` → `assessRegister`, so a CI job posting twelve vendors and a person adding a row produce the same register and the same findings; this codebase has now found the two-doors-disagree bug four times (SaaS posture enrich, device posture fold, sync-vs-ingest fold, and this). Rows upsert by a slug of the NAME, so a nightly re-post updates rather than doubling the register. The whole register is re-assessed on every write (vendor risk is a property of the PORTFOLIO, so assessing only the edited row leaves findings standing for vendors since fixed) and a DELETE re-assesses too, so a relationship that has ended stops raising risks about itself. Grounded §10: every field is DECLARED — nothing about a name or category says whether a vendor holds personal data or signed a DPA — an **empty register is reported as empty, never as clean** (a company with nothing written down and one with no vendor risk are identical in a findings list), and an **unowned vendor stays unowned** rather than defaulting to the workspace owner, which would name somebody who never agreed to be accountable (`Asset.Owner`'s rule, ADR 0028 G1). NEVER-reviewed is counted separately from reviewed-long-ago, and **no portfolio score is emitted** — one figure blending "we hold their SOC 2" with "nobody has looked since 2024" would RISE as a customer recorded fewer vendors. `GET /v1/vendors` · `POST /v1/vendors` · `DELETE /v1/vendors/{id}`, rendered above the findings on `/posture` and guarded by `uicheck`. |
| `internal/deviceposture` | **endpoint / device posture (MDM-lite)** — the Vanta device-monitoring "finding issues" capability; employee laptops/phones are an asset class. `Assess(devices []Device)` surfaces grounded device-posture findings: `disk-unencrypted` (high; SOC2 CC6.7, HIPAA 164.312(a)(2)(iv), NIST SC-28), `tampered` (jailbroken/rooted, high), `os-end-of-life` (high; SI-2), `no-screen-lock` (medium; AC-11), `firewall-off` (medium; SC-7), `no-edr` (medium; SI-3), `auto-update-off` (low). Snapshot-driven, LLM-free, grounded (§10) — a compliant fleet yields ZERO findings; a missing field never invents risk. Live driver: `POST /v1/devices/ingest`. **The FETCH half is now built too** (`internal/mdm`): `Kandji`, `Jamf` (Jamf Pro API, API-client or bearer) and `Intune` (Graph, borrowing the onboarded M365 connection's token when the config has none) each read the live inventory and hand `deviceposture.Device` records to the SAME `ingestDevices` the posted snapshot uses. Configured per tenant (`Tenant.MDM`, `GET/PUT /v1/settings/mdm`, credentials sealed) · on demand (`POST /v1/devices/sync`) · and on EVERY monitoring pass (`runner.syncDevices` via `Service.MDMFetcher`, the same factory as the button). The discipline that makes a fetcher safe to be wrong: a pointer setting is asserted ONLY where the provider reported it, so a misread field lands as "not assessed" and never as a finding; each fetcher DECLARES the settings its provider cannot report per device (`Report.ProviderLimits` → `checks_not_run`) and NAMES devices it could not fully read (`Report.Unread`); an undecodable 2xx (a login page) is an ERROR, never an empty fleet. `OSEndOfLife` is not set by any fetcher (no version table) — a documented gap, not a silent one |
| `internal/hris` | **the JOINER / LEAVER join** — employment records from the tenant's HR system correlated against the accounts its identity provider holds; the surface docs/integrations.md §3.2 called "the one thing we have nothing for". `operate` sees ACCOUNTS; only HR sees EMPLOYMENT, so a stale-account check catches a leaver ninety days late and this catches them the day HR records the departure. Two unified-API fetchers (`Merge` — Merge.dev, key + linked-account token; `Finch` — directory + batched individual/employment reads), deliberately NOT fifty individual HRIS integrations. The roster is STORED per source (`Store.ReplaceEmployees`/`ListEmployees`, 6-place wired, tenant-isolation asserted — it is another company's staff PII) so the join runs on EVERY monitoring pass (`runner.OperateRunner.Employees`, reporting `hris` in `ToolsRan` only when it actually ran over a non-empty roster and non-empty accounts) as well as on `POST /v1/hris/sync`. `Correlate` emits `hris::leaver-with-active-account` (high; CRITICAL if the account holds an admin role — SOC 2 CC6.2/CC6.3, ISO A.5.18, NIST AC-2(3)/PS-4, HIPAA 164.308(a)(3)(ii)(C), PCI 8.2.5) and `hris::account-without-hr-record` (low — "no record", never "rogue": service accounts legitimately have none, and the ask is to record an owner). Grounded (§10) as refusals: identity is matched on EMAIL EQUALITY against addresses the HRIS itself asserts (work + personal), never on a name or a resemblance — a wrong merge here sends someone to suspend a current employee; a FUTURE end date is a scheduled departure, not a leaver; an empty roster, an empty account list or a roster without addresses each land in `Report.ChecksNotRun` rather than reading as "offboarding works". The `Report` counts the SUCCESSES too (`LeaversDeprovisioned`) because that is what an auditor wants to see beside the failures. A leaver finding reuses `operate`'s `account_suspend` runbook, so it promotes to the live HITL-gated suspend on Okta / Google Workspace / M365 (`remediate.liveIdentityMutation`) — the sync proposes against the WORKSPACE asset (`proposeForFindingsOn`) so that promotion is reachable from the on-demand door too. Configured via `Tenant.HRIS` (`GET/PUT /v1/settings/hris`), rendered on `/posture` as "Joiners & leavers" and in Settings → People & devices, guarded by `uicheck` |
...
password â email-based invites are the next step). **A fourth role, `employee`, is the NARROWEST seat** — a colleague asked to do their security training and acknowledge the policies, and who has no business seeing the security estate. It exists because the training programme is unusable without it: asking forty people to complete their modules meant inviting forty MEMBERS, and a member reads every finding, attack path and pentest report, starts scans and approves fixes — so nobody sends that invite and the programme every framework requires sits permanently unstarted. Enforced by an ALLOWLIST in the auth middleware (`internal/platformapi/employee_scope.go`), and the DIRECTION is the point: with a denylist every endpoint added later is exposed to the whole company by default, and whoever adds it has no reason to think about employees. `TestEmployeeSeatIsRefusedEverythingItIsNotExplicitlyGiven` drives all 248 registered routes through the real gate and fails on any the seat reaches — removing the gate reproduces as "an employee seat reached 244 endpoint(s)". `POST /v1/training/record` is deliberately NOT allowed: recording that a COLLEAGUE trained elsewhere is an administrative act about somebody else. **The allowlist is the gate, not the whole boundary**: `GET /v1/training` and `GET /v1/program` are admitted because the employee needs their OWN rows from them, and both handlers were then returning the whole company to that seat — every colleague's training status and off-roster name, every draft policy, every acknowledgement — the estate arriving through the two doors left open. Both handlers now CUT the response to the acting employee (`rosterOnly`/`completionsOf`, `policiesForEmployee`: published only, own ack only) and say so with `scope: "self"`; the two pages branch on that field, never on the size of what came back (a company of one and a colleague's view are the same list), and the employee branch renders none of the owner's controls, since a control the API refuses reads as a broken page rather than a scoped one. Mutation-verified server-side and guarded by `uicheck`. When a route is added to the allowlist, ask what the handler RETURNS to that seat, not only whether it may be called. **The seats are minted from the HRIS roster in one act** (`internal/platformapi/invite_roster.go`: `GET /v1/auth/invite-roster` previews, `POST` seats; owner-only; Settings → Team panel) — the programme was unusable at any real size while the only door was one invite form per person, and the HRIS join already knows exactly who works here. Six refusals, all tested: only ACTIVE employment is seated (a leaver's seat outlives them; a pending hire's credential would sit in a mailbox they do not have yet — both NAMED as skipped with the reason and the start date); only the EMPLOYEE role is ever created and there is no parameter to ask for another; an address holding ANY seat is never re-invited (the credential would reset) nor downgraded, and is reported with its role; no work email means no seat and the person is named by HRIS name and id rather than dropped; an EMPTY roster is refused (409 `no_roster`), not answered "0 invited"; and a run is bounded at 200 with the remainder reported. Idempotent by construction. ONE provisioning path (`provisionSeat`/`deliverInvite`) serves the single invite and the roster invite, so the two doors cannot drift on what a fresh account looks like — the credential is mailed and NOT returned when SMTP is configured, returned once for relay when it is not. The panel shows the PLAN before the act and names who is not seated; `uicheck` pins both and bans any role control on it. The frontend returns a SEPARATE shell BEFORE the estate fetches (guarded by `uicheck`), because rendering the console with pages hidden would fire seven calls this account is refused, each falling back to its empty default — drawing a risk rating of zero and a nav of links that render BLANK, and blank reads as "my data vanished" rather than "not for you". **A third role, `auditor`, is READ-ONLY** — the external SOC 2 / ISO auditor, a CPA-firm partner,
