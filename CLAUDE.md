...
| `internal/crossdetect` | the **unified cross-detection** adapter: turns the store's flat per-tenant finding list into `correlate.Asset` buckets (grouped by the asset type inferred from each finding's tool) so `correlate.Correlate` can build cross-surface attack paths — a finding on one surface bridging, via a real shared entity (key/ARN/host/IP/bucket), to a crown jewel on another. Surfaced at `GET /v1/attack-paths` (`platformapi/crossdetect.go`) + the frontend `/attack-paths` page + a dashboard banner. Orchestration glue over the existing `correlate` engine — adds no detection, links only on a real shared entity (§10/§13 hold) |
