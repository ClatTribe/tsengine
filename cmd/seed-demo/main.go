...
func main() {
	...
	findings := []types.Finding{
		...
		// access key (web entry point), and that SAME key (the bridge entity AKIA3TZQ7WJVB2XMR4KD)
	...
	actions := []platform.Action{
		{ID: "act-001", TenantID: tid, FindingID: "f-002", ConnectionID: "conn-gh", Kind: platform.ActOpenPR, Tier: 2, Status: platform.ActPendingApproval, Title: "Upgrade xz-utils past the backdoored range", Payload: map[string]any{"summary": "Bumps the transitive xz-utils dependency to a patched version and re-locks go.sum. CI passed on the branch.", "target": "payments-api", "full_name": "northwind-labs/payments-api", "head": "tensorshield/bump-xz-utils", "base": "main"}, CreatedAt: ago(24 * time.Hour)},
		{ID: "act-002", TenantID: tid, FindingID: "f-005", ConnectionID: "conn-aws", Kind: platform.ActApplyConfig, Tier: 2, Status: platform.ActPendingApproval, Title: "Make northwind-invoices bucket private", Payload: map[string]any{"summary": "Removes the public-read statement from the bucket policy and enables Block Public Access. Reversible.", "target": "arn:aws:s3:::northwind-invoices", "remediation_type": "s3_block_public_access", "bucket": "northwind-invoices"}, CreatedAt: ago(23 * time.Hour)},
